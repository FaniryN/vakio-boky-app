...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("🔐 Réponse complète du login:", data);

      if (response.ok) {
        // STRUCTURE CORRECTE POUR LE HOOK AUTH
        const userData = {
          token: data.token,
          user: {
            id: data.user.id,
        ...
        console.log("✅ UserData à sauvegarder:", userData);
        
        // CORRECTION : Toujours se souvenir de l'utilisateur
        login(userData, true);

        navigate(from, { replace: true });
      } else {
        alert(data.error || `Erreur ${response.status}: ${response.statusText}`);
      }
    } catch (err) {
