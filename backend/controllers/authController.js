...
const login = async (req, res) => {
  try {
    ...
    console.log("✅ Utilisateur trouvé:", { id: user.id, nom: user.nom, email: user.email });
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log("🔍 Hash stocké (premiers 30 chars):", user.mot_de_passe.substring(0, 30));
    }
...
const resetPassword = async (req, res) => {
  try {
    ...
    console.log(`✅ Mot de passe réinitialisé pour l'utilisateur ID: ${decoded.userId}`);
