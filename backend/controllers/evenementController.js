...
export const getEvents = async (req, res) => {
  try {
    console.log("📅 Récupération événements publics");
...
export const createEvent = async (req, res) => {
  try {
    ...
    if (!title || !event_date || !location) {
      return res.status(400).json({
        success: false,
        error: "Titre, date et lieu requis"
      });
    }
...
export const getDetailById = async (req, res) => {
  try {
    ...
    console.error('❌ Erreur getDetailById:', error);
    res.status(500).json({
      success: false,
      error: "Erreur serveur"
    });
...
// GET /api/admin/events - CORRIGÉ
export const getAllEventsAdmin = async (req, res) => {
  try {
    console.log("👑 [Admin] Récupération tous les événements");
    ...
    console.error("❌ Erreur getAllEventsAdmin:", error);
    res.status(500).json({
      success: false,
      error: "Erreur serveur"
    });
...
export const rejectEvent = async (req, res) => {
  try {
    ...
    if (!reason || reason.trim().length < 10) {
      return res.status(400).json({
        success: false,
        error: "Motif requis (10 caractères min)"
      });
    }
    ...
    console.error("❌ Erreur rejectEvent:", error);
    res.status(500).json({
      success: false,
      error: "Erreur serveur"
    });
...
export const featureEvent = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: featured ? "Mis en avant" : "Retiré",
      event: event,
    });
  } catch (error) {
    console.error("❌ Erreur featureEvent:", error);
    res.status(500).json({
      success: false,
      error: "Erreur serveur"
    });
